...
def _get_style_description_from_image_style(image_style: Dict) -> str:
    ...
    if not image_style or not isinstance(image_style, dict):
        return "写实风格，8K超高清，最高清晰度，细节丰富锐利，16英寸屏幕绝对清晰"
    ...
    if style_type == "realistic":
        ...
        if subtype == "photorealistic":
            return "16K UHD超高清画质（最高清晰度，细节锐利无模糊，16英寸屏幕绝对清晰），RAW格式无损输出，85mm f1.4定焦镜头，ISO 100，快门1/125s，浅景深虚化（虚化半径2.0px），全局光照（照度1200lux），HDR10+动态范围（峰值亮度1000nit）；面部建模：8K超写实面部拓扑，柔肤滤镜（强度35%），虹膜动态高光追踪（高光直径0.8mm，反射率82%），1.48秒眨眼周期，上眼睑下垂14度，下眼睑微颤0.45HZ；毛发系统：2200+根独立发丝动力学模拟，发根固定系数0.96，发梢飘动幅度7-11cm，发丝摩擦系数0.32；服装材质：棉质面料（弹性模量180GPa，刚性系数9.2），衣物褶皱自然分布（褶皱深度0.3-0.8cm）；环境参数：漫反射柔光（色温5600K），颈部后方点状风源（风速1.1m/s，湍流强度5.5%）；镜头设定：视角固定（水平视角60度），仅头发、衣物随动力学飘动，眼睛按参数完成眨眼动作，无多余动态干扰；必须保证画面最高清晰度，细节锐利"
        if subtype == "game_realistic":
            return "UE5.3引擎渲染，光线追踪全局光照（Lumen质量等级超高），Nanite微多边形几何体，4K 120Hz超高清画质（最高清晰度，16英寸屏幕绝对清晰），HDR渲染（色域DCI-P3 100%）；场景参数：动态体积雾（密度0.08，散射系数0.12），环境光遮蔽（采样率256x），物理级材质反射（金属反射率92%，玻璃折射率1.52）；人物建模：8K高精度拓扑，皮肤纹理扫描级还原（毛孔直径18-22μm），肌肉动力学模拟（面部52个表情肌独立驱动）；镜头分镜：广角叙事镜头（焦距24mm，水平视角84度），2.39:1宽银幕画幅，电影级运镜（缓慢推镜速度0.8m/s）；动态设定：地面物理摩擦系数0.65，人物行走步幅75cm，手臂摆动幅度30度，衣物随动作自然形变（形变系数0.78）；光影参数：主光色温6500K，补光色温4300K，光比3:1，阴影柔化半径0.9px；必须保证4K+超高分辨率，细节锐利清晰"
        if subtype == "cinematic_realistic":
            return "IMAX 70mm胶片质感，2.39:1宽银幕画幅，4K超高清画质（最高清晰度，16英寸屏幕绝对清晰），胶片颗粒度ISO 200（颗粒尺寸0.15μm），诺兰式高对比度光影（对比度180:1）；镜头参数：中长焦叙事镜头（焦距50mm，光圈f2.0），黄金分割构图（九宫格参考线开启），镜头畸变校正（畸变系数-0.02）；光影设定：聚光主光（照度1500lux，光斑直径1.2m），环境漫反射（照度350lux），明暗过渡梯度256级；动态参数：人物动作帧率24fps，自然呼吸幅度（胸腔起伏0.8-1.2cm，呼吸频率18次/分钟），面部微表情（嘴角自然上扬0.5度，眼神轻微游动0.3Hz）；材质细节：皮革材质（粗糙度0.25，光泽度75%），织物纹理（纱线密度32根/cm²），金属划痕细节（划痕长度0.5-2cm，深度0.03mm）；必须保证画面高清锐利，细节丰富"
        if subtype == "hyper_realistic":
            return "16K 60Hz超高清画质（最高清晰度，放大400%无模糊，细节极致锐利，16英寸屏幕绝对清晰），微距级细节渲染，冷军级材质还原；面部细节：皮肤油脂光泽（光泽度68%），毛细血管可见（直径0.02mm，分布密度12根/cm²），唇纹深度0.05mm，唇色渐变（从唇峰到唇角色阶差12级）；毛发细节：眉毛1200+根独立建模，眉毛细度0.08mm，生长方向自然（眉峰向上倾斜12度），胡须生长密度8根/mm²；材质参数：金属材质（镜面反射率98%，划痕反射强度0.7），玻璃材质（透光率92%，折射角1.517），布料材质（纤维直径12μm，编织密度40根/cm²）；光影参数：自然柔光（色温5500K，照度1100lux），无明显阴影（阴影密度0.15），全局反光强度0.2；动态设定：无多余动态，仅面部微表情（眼神眨眼周期1.5秒，鼻翼微颤0.3Hz）；必须保证最高清晰度，无任何模糊"
        return "UE5.3引擎渲染，光线追踪全局光照（Lumen质量等级超高），4K 120Hz超高清画质（最高清晰度，16英寸屏幕绝对清晰）"
    if style_type == "anime":
        subtype = image_style.get("subtype", "genshin")
        if subtype == "hayao_miyazaki":
            return "4K超高清手绘赛璐璐风格（最高清晰度，线条精细清晰锐利，16英寸屏幕绝对清晰），传统手绘笔触（笔刷大小1.6px，笔压强度0.7），水彩晕染效果（晕染半径2.5px，透明度65%）；色彩参数：暖色调（色温4800K），色彩饱和度72%，对比度120:1，色阶过渡256级；线条设定：轮廓线宽度1.2px，线条平滑度95%且清晰锐利，内部线条宽度0.5px，线条颜色深棕色（RGB 52,32,18）；分镜参数：中景叙事镜头（焦距35mm，视角75度），平视构图，帧速率24fps，画面抖动幅度0.08px（模拟手绘轻微抖动）；人物细节：眼睛建模（虹膜直径8px，高光点2个，高光大小0.4px），发丝线条（单根宽度0.2px，分组飘动，每组5-8根）；场景细节：草地纹理（草叶长度2.5-4px，密度15根/cm²），水面波纹（波纹幅度0.4px，频率2Hz），光影设定：漫反射柔光（照度800lux），无明显阴影，氛围光强度0.3；必须保证4K超高清晰度"
        if subtype == "makoto_shinkai":
            return "4K HDR超高清画质（最高清晰度，通透锐利，16英寸屏幕绝对清晰），极致通透空气感（空气透视强度0.25），丁达尔体积光（光束直径1.0px，强度0.8，色温6200K）；色彩参数：高饱和通透色调（饱和度82%，对比度130:1），天空渐变（从顶部RGB 102,187,255到底部RGB 255,242,224，渐变过渡100级）；镜头参数：广角场景镜头（焦距20mm，视角90度），特写与全景交替（特写镜头焦距85mm，光圈f1.8），镜头光晕（光晕强度0.3，光晕直径3.2px）；人物细节：面部线条（轮廓线宽度1.0px，线条锐利度90%且清晰），眼睛高光（动态高光追踪，随视角移动，高光反射率85%）；粒子特效：樱花粒子（粒子大小0.5-0.8px，下落速度0.5cm/s，旋转速度0.3Hz），光斑粒子（大小0.2-0.35px，亮度0.9）；动态设定：发丝飘动（幅度5-9cm，风源风速0.8m/s），裙摆飘动（形变系数0.8，摆动频率1.2Hz）；必须保证4K超高清晰度，画面锐利"
        if subtype == "shonen_jump":
            return "4K超高清热血动漫风格（最高清晰度，轮廓线精细清晰锐利，16英寸屏幕绝对清晰），硬朗轮廓线（宽度1.4px，线条锐利度98%，颜色纯黑RGB 0,0,0）；色彩参数：高对比平涂上色（对比度150:1，饱和度85%），阴影平涂（阴影色阶差3级，无渐变）；分镜参数：动态冲击构图（广角镜头焦距18mm，视角95度），速度线特效（速度线宽度0.7-2px，密度8根/cm，角度45度），动态模糊（模糊强度0.8，模糊方向与动作方向一致）；人物细节：眼睛建模（虹膜直径9px，高光点1个，大小0.5px，眼神锐利），肌肉线条（线条宽度0.8px，阴影加深0.4）；动态参数：战斗动作帧率30fps，肢体摆动幅度（手臂摆动45度，腿部踢动60度），头发飘动（幅度10-15cm，风源风速1.5m/s，湍流强度8%）；特效参数：战斗光效（光效直径3-5px，亮度0.9，颜色渐变RGB 255,0,0到RGB 255,128,0）；必须保证4K超高清晰度"
        if subtype == "cyber_anime":
            return "4K超高清赛博朋克动漫（最高清晰度，线条精细清晰锐利，16英寸屏幕绝对清晰），冷峻工业线条（宽度1.3px，线条硬度95%，颜色RGB 0,128,255）；色彩参数：低饱和冷色调（饱和度55%，对比度140:1），霓虹光效点缀（霓虹色RGB 0,255,255、RGB 255,0,255，亮度0.9）；分镜参数：近景特写镜头（焦距60mm，光圈f2.2），俯拍/仰拍交替构图，镜头畸变（畸变系数0.03）；人物细节：机械义体建模（金属线条宽度1.0px，反光强度0.85），皮肤纹理（偏冷色，RGB 220,220,220，粗糙度0.2）；特效参数：故障艺术特效（故障偏移0.35px，频率1.5Hz，颜色分离RGB偏差5级），全息投影（投影分辨率1080p，透明度70%，投影模糊半径0.25px）；动态设定：义体关节转动（转动角度0-180度，转动速度30度/秒），发丝飘动（幅度6-10cm，风源风速1.0m/s）；必须保证4K超高清晰度"
        if subtype == "genshin":
            return "4K超高清高精二次元渲染（最高清晰度，线条精细清晰锐利，16英寸屏幕绝对清晰），PBR卡通材质（金属度0.15，粗糙度0.3，高光强度0.8）；色彩参数：通透渐变色调（饱和度78%，对比度125:1），人物肤色渐变（面部RGB 255,230,210到颈部RGB 255,220,200）；镜头参数：中近景人物构图（焦距50mm，视角70度），浅景深虚化（虚化半径1.5px），镜头光晕（强度0.25，颜色RGB 255,255,200）；人物细节：眼睛建模（虹膜直径8.5px，高光点3个，大小0.35-0.5px，动态高光随视角移动），发丝建模（1800+根独立线条，单根宽度0.18px，分组飘动，每组6-9根）；粒子特效：元素粒子（大小0.35-0.7px，亮度0.85，下落速度0.4cm/s），衣物光泽（光泽度70%，反光范围1.0px）；动态设定：呼吸动作（胸腔起伏0.7-1.1cm，频率17次/分钟），发丝飘动（幅度4-8cm，风源风速0.7m/s），裙摆摆动（幅度8-12cm，摆动频率1.0Hz）；必须保证4K超高清晰度"
        if subtype == "josei":
            return "4K超高清女性向治愈动漫（最高清晰度，线条精细清晰柔和，16英寸屏幕绝对清晰），纤细柔和线条（宽度0.8px，线条平滑度98%且清晰，颜色RGB 80,80,80）；色彩参数：莫兰迪低饱和色调（饱和度60%，对比度100:1），浅色系为主（底色RGB 250,248,245）；分镜参数：抒情慢镜头（帧速率18fps），浅景深氛围（虚化半径2.0px），平视微仰构图（视角5度）；人物细节：面部细节（唇纹深度0.03mm，腮红范围直径1.5cm，颜色RGB 255,180,180，透明度50%），发丝线条（单根宽度0.15px，柔软度0.8，飘动幅度3-6cm）；光影参数：温柔柔光布光（色温4500K，照度700lux），阴影柔化半径1.0px，阴影密度0.1；动态设定：轻微头部晃动（幅度1-2度，频率0.2Hz），眼神缓慢游动（速度0.5px/s），无剧烈动作；必须保证4K超高清晰度"
        return "4K超高清高精二次元渲染（线条精细，16英寸屏幕绝对清晰），PBR卡通材质，通透渐变色调，最高清晰度"
    if style_type == "ink_painting":
        subtype = image_style.get("subtype", "shanshui")
        if subtype == "shanshui":
            return "4K超高清宣纸水墨（最高清晰度，笔触清晰锐利，16英寸屏幕绝对清晰），焦浓重淡清五色墨韵（焦墨浓度90%，浓墨75%，重墨60%，淡墨40%，清墨25%）；笔触参数：毛笔笔触（笔刷大小2-5px，笔压强度0.6-0.9，笔触羽化0.4px），晕染效果（晕染半径3-5px，渗透强度0.7）；构图参数：高远+深远结合构图（上半部分高远视角，下半部分深远视角），留白比例35%，画面比例16:9；山水细节：山峰线条（线条宽度1.5-2.5px，苍劲度90%，清晰锐利），树木建模（树干线条宽度1.2-2.0px，枝叶笔触大小0.6-1.2px，密度12根/cm²）；光影参数：水墨明暗对比（对比度110:1），无明显色彩，仅墨色层次（256级墨色过渡）；动态设定：无动态，静态水墨，墨色晕染自然，笔触流畅清晰，意境静谧；必须保证4K超高清晰度，笔触锐利"
        if subtype == "xieyi":
            return "4K超高清大写意水墨（最高清晰度，笔触清晰有力，16英寸屏幕绝对清晰），泼墨技法（泼墨范围5-12cm，墨色浓度50-80%，渗透强度0.85）；笔触参数：自由洒脱笔触（笔刷大小3-7px，笔压强度0.4-0.8，笔触无规则羽化0.25-0.5px）；构图参数：极简留白构图（留白比例60%），不对称构图（主体偏左30%）；细节参数：主体笔触（线条宽度2.2-4px，形神兼备，线条扭曲度15%，清晰可见），墨色层次（焦淡对比强烈，浓墨占比30%，淡墨占比70%）；技法参数：飞白笔触（飞白长度2-5cm，飞白密度0.6），枯笔效果（枯笔占比25%，笔刷干燥度75%）；画面效果：墨色氤氲渗透，笔触自然灵动清晰，无多余细节，气韵生动；必须保证4K超高清晰度"
        if subtype == "gongbi":
            return "4K超高清工笔重彩水墨（最高清晰度，线条极致精细清晰，16英寸屏幕绝对清晰），精细白描线条（线条宽度0.4-1.0px，线条平滑度99%，线条锐利度95%）；墨色参数：淡墨晕染底色（墨色浓度30%，晕染半径2.0px，渗透强度0.6）；色彩参数：淡雅工笔设色（饱和度45%，对比度90:1），肤色RGB 255,240,225，衣纹色彩RGB 180,200,220，透明度80%；细节参数：衣纹线条（每厘米8-10根线条，线条间距0.1-0.2cm，清晰可见），发丝细节（单根线条宽度0.12px，密度10根/mm²），面部细节（眉毛细度0.08px，眼睫毛长度0.6px，密度6根/mm²）；构图参数：对称严谨构图（对称偏差≤1%），主体居中，背景简洁（墨色浓度20%）；画面效果：线条工整流畅，细节精致入微清晰，色彩淡雅高级，古典韵味浓厚；必须保证4K超高清晰度"
        if subtype == "wuxia":
            return "4K超高清武侠水墨（最高清晰度，笔触清晰锐利，16英寸屏幕绝对清晰），飞白笔触（飞白长度3-8cm，飞白密度0.7，笔刷大小1.8-3.5px）；墨色参数：浓淡对比强烈（浓墨浓度80%，淡墨浓度30%，对比度140:1）；构图参数：侠客剪影构图（主体占画面40%，背景山水占60%），动态倾斜构图（倾斜角度10度）；细节参数：侠客衣袍（线条宽度1.5-2.2px，褶皱深度0.5-1.0cm，飘动幅度10-15cm，清晰可见），刀剑线条（宽度1.8px，锐利度98%，反光强度0.7）；动态参数：刀光剑影特效（线条宽度1.1px，长度5-10cm，亮度0.8，动态模糊强度0.6），侠客动作（身体倾斜30度，衣袖飘动频率1.5Hz）；氛围参数：水墨雾气（密度0.1，模糊半径1.5px），江湖氛围感拉满；必须保证4K超高清晰度"
        if subtype == "modern_ink":
            return "4K超高清新中式现代水墨（最高清晰度，线条精细清晰锐利，16英寸屏幕绝对清晰），传统毛笔笔触（笔刷大小1.5-4.5px，笔压强度0.5-0.8）+ 现代几何构图（几何图形占比35%）；墨色参数：撞色水墨（墨色浓度60%，撞色RGB 255,100,100、RGB 100,100,255，透明度70%）；线条参数：简约有力线条（宽度1.1-1.8px，线条硬度85%，清晰锐利），几何线条（宽度1.5px，直线度99%）；构图参数：几何解构构图（分割比例1:1:2），留白比例25%；细节参数：水墨与撞色融合（融合过渡10级，边缘模糊半径0.6px），现代元素点缀（线条几何图形，大小3-5cm）；光影参数：现代光影（色温5000K，照度900lux），水墨反光强度0.3，撞色区域亮度0.85；必须保证4K超高清晰度"
        return "4K超高清宣纸水墨（16英寸屏幕绝对清晰），焦浓重淡清五色墨韵，毛笔笔触清晰锐利"
    if style_type == "oil_painting":
        subtype = image_style.get("subtype", "classic_oil")
        if subtype == "impressionist":
            return "4K超高清博物馆级印象派油画（最高清晰度，笔触精细清晰可见，16英寸屏幕绝对清晰），厚涂肌理笔触（笔触厚度0.8-1.5mm，笔触大小2-5px，笔触密度15根/cm²）；颜料参数：莫奈同款颜料（透明度65%，色彩饱和度78%），色彩并置技法（相邻色块色差15级，无明显过渡）；光影参数：自然光漫反射（色温5800K，照度1000lux），光影随时间变幻（光影偏移速度0.08px/s）；构图参数：中景风景构图（主体占画面50%，背景占50%），视角平视，画幅比例4:3；细节参数：水面光影（波纹幅度0.3-0.6cm，光影反射率80%），树叶笔触（大小0.4-1.0px，密度20根/cm²）；画面效果：笔触富有节奏感清晰可见，色彩朦胧柔和，光影灵动，充满自然诗意，无明显轮廓线；必须保证4K超高清晰度"
        if subtype == "rococo":
            return "4K超高清洛可可宫廷油画（最高清晰度，细节精致清晰，16英寸屏幕绝对清晰），细腻光滑笔触（笔触厚度0.3-0.6mm，笔触大小0.7-2px，笔触密度25根/cm²）；色彩参数：马卡龙粉嫩色调（饱和度70%，对比度100:1），主色调RGB 255,220,230、RGB 240,220,255，背景色RGB 255,250,245；细节参数：繁复卷草纹装饰（花纹大小1-2cm，密度8个/cm²，线条宽度0.4-0.8px，清晰可见），贵族服饰细节（蕾丝纹理密度30根/mm²，珠宝光泽度90%，反光强度0.85）；光影参数：柔光布光（色温4800K，照度850lux），阴影柔化半径1.3px，阴影密度0.15；构图参数：对称优雅构图（对称偏差≤0.5%），主体居中，背景宫廷装饰占比40%；人物细节：肌肤质感（细腻度98%，光泽度75%，毛孔不可见）；必须保证4K超高清晰度"
        if subtype == "renaissance":
            return "4K超高清文艺复兴古典油画（最高清晰度，细节精细清晰，16英寸屏幕绝对清晰），多层薄涂技法（涂层厚度0.2-0.4mm，层数8-12层）；色彩参数：沉稳典雅色调（饱和度65%，对比度120:1），主色调RGB 180,150,120、RGB 120,100,80，明暗过渡256级；构图参数：黄金比例构图（比例1:1.618），宗教/古典人物题材，主体占画面55%；人物细节：面部建模（8K精度，肌肉线条自然清晰，表情庄重），衣物纹理（丝绸材质，光泽度85%，褶皱深度0.5-1.2cm）；光影参数：柔和明暗过渡（chiaroscuro明暗对比150:1），主光照度1200lux，补光照度400lux；画面效果：博物馆级质感，庄重典雅，充满古典艺术神圣感，笔触细腻无明显痕迹；必须保证4K超高清晰度"
        if subtype == "baroque":
            return "4K超高清巴洛克油画（最高清晰度，光影细节清晰锐利，16英寸屏幕绝对清晰），伦勃朗式强烈明暗对比（对比度200:1），厚重油彩肌理（笔触厚度1.0-2.0mm，笔触大小2.8-5.5px）；光影参数：戏剧性聚光光影（主光光斑直径1.5m，照度1800lux，补光照度250lux）；构图参数：动态张力构图（主体倾斜15度，视线引导线指向主体），画幅比例3:4；细节参数：人物衣物（天鹅绒材质，粗糙度0.2，光泽度88%），金属装饰（反射率95%，划痕细节0.3-0.8cm）；动态氛围：画面充满戏剧冲突，人物表情夸张（眼神锐利，嘴角紧绷），光影塑造强烈情绪；笔触效果：笔触浑厚有力，油彩堆积明显，画面立体感极强；必须保证4K超高清晰度"
        if subtype == "abstract_oil":
            return "4K超高清立体主义抽象油画（最高清晰度，色块边缘锐利清晰，16英寸屏幕绝对清晰），毕加索风格几何解构（几何图形占比80%，图形类型：三角形、圆形、矩形）；色彩参数：强烈色彩碰撞（饱和度90%，对比度180:1），主色调RGB 255,0,0、RGB 0,0,255、RGB 255,255,0，色块边缘锐利；笔触参数：粗犷豪放笔触（笔触厚度0.8-1.8mm，笔触大小3.5-7px，笔触方向无规则）；构图参数：多维视角构图（俯视+平视结合，视角偏差30度），画面分割比例2:3:5；细节参数：色块叠加（叠加层数5-8层，透明度50-70%），线条交织（线条宽度1.4-2.2px，密度10根/cm）；画面效果：先锋艺术感拉满，视觉冲击力强，无明确主体，自由艺术表达；必须保证4K超高清晰度"
        return "4K超高清经典古典油画（最高清晰度，细节丰富清晰，16英寸屏幕绝对清晰），厚重油彩肌理（笔触厚度0.7-1.3mm，笔触大小1.4-4px）；光影参数：伦勃朗光影塑造（明暗对比160:1），主光色温5200K，照度1100lux；色彩参数：浓郁沉稳色调（饱和度70%，对比度130:1），色彩过渡自然（256级色阶）；细节参数：人物面部（8K精度，皮肤纹理细腻清晰，毛孔直径20μm），风景细节（树木纹理密度18根/cm²，岩石纹理深度0.5-1.0mm）；技法参数：多层覆涂技法（涂层厚度0.3-0.5mm，层数6-10层）；画面效果：博物馆收藏级质感，光影立体饱满，构图严谨典雅，艺术厚重感极强；必须保证4K超高清晰度"
    if style_type == "cyberpunk":
        subtype = image_style.get("subtype", "night_city")
        if subtype == "night_city":
            return "16K UHD超高清赛博朋克2077夜之城风格（最高清晰度，16英寸屏幕绝对清晰），UE5.3光线追踪渲染（Lumen质量超高，光线反弹次数8次）；环境参数：雨夜湿滑地面（反射率90%，反光模糊半径0.25px，积水深度0.2-0.5cm），密集霓虹全息广告（分辨率4K，亮度1200nit，闪烁频率2.5Hz）；色彩参数：高对比荧光撞色（主色RGB 0,255,255、RGB 255,0,255，饱和度95%，对比度200:1）；细节参数：机械义体（金属材质反射率95%，关节转动角度0-180度，纹理划痕0.3-1.0cm，清晰可见）；特效参数：故障艺术特效（故障偏移0.6px，频率2.0Hz，RGB颜色分离8级），体积雾（密度0.12，散射系数0.15，色温6500K）；镜头参数：广角都市分镜（焦距20mm，视角90度），2.39:1宽银幕画幅，镜头畸变系数0.04；动态参数：行人动作（步幅65cm，行走速度1.2m/s），全息投影动态（播放帧率30fps，透明度75%）；必须保证16K最高清晰度，细节锐利"
        if subtype == "blade_runner":
            return "4K超高清复古未来赛博朋克（最高清晰度，16英寸屏幕绝对清晰），银翼杀手2049风格，低饱和冷灰底色（饱和度45%，对比度170:1）；色彩参数：霓虹蓝橙撞色（RGB 0,128,255、RGB 255,128,0，亮度0.85）；环境参数：潮湿空气体积光（光束直径1.3px，强度0.7，密度0.1），巨型全息投影（分辨率2K，投影尺寸5×8m，透明度80%）；镜头参数：慢镜头叙事分镜（帧速率12fps），超广角城市构图（焦距16mm，视角100度），胶片颗粒感（ISO 400，颗粒尺寸0.2μm）；细节参数：城市肌理（破败墙面划痕3-8cm，墙面粗糙度0.8，清晰可见），地面反光（反射率75%，模糊半径0.35px）；动态参数：缓慢运镜（推镜速度0.3m/s），雨滴下落（速度5m/s，雨滴大小0.3-0.5mm，密度20滴/cm²）；氛围参数：反乌托邦压抑氛围，画面冷峻疏离，未来孤独感拉满；必须保证4K超高清晰度"
        if subtype == "anime_cyber":
            return "4K超高清日式赛博朋克动画（最高清晰度，线条精细清晰锐利，16英寸屏幕绝对清晰），阿基拉+攻壳机动队画风，手绘线条+未来科技元素（线条宽度1.2px，锐利度95%）；色彩参数：暗调冷色为主（饱和度50%，对比度150:1），霓虹光条点缀（RGB 0,255,255，宽度1.0px，亮度0.9）；场景细节：东京新宿街头（楼宇高度80-120m，广告牌密度12个/100m²，中式+日文牌匾）；镜头参数：动态街头分镜（帧速率24fps），近景跟拍（焦距35mm，跟拍速度1.5m/s）；人物细节：机甲建模（金属线条宽度0.8px，反光强度0.8），人物服装（皮质材质，粗糙度0.25，褶皱深度0.4-0.9cm）；特效参数：霓虹光效（光晕半径1.3px，闪烁频率1.8Hz），烟雾特效（密度0.08，模糊半径1.0px）；动态参数：机甲关节转动（速度45度/秒），人物行走步幅70cm；必须保证4K超高清晰度"
        if subtype == "dystopian":
            return "4K超高清反乌托邦废土赛博朋克（最高清晰度，细节清晰锐利，16英寸屏幕绝对清晰），破败未来城市肌理（楼宇破损率40%，墙面裂缝宽度0.5-2.0cm，废墟堆积高度5-10m）；色彩参数：灰调压抑底色（饱和度35%，对比度160:1），少量霓虹破碎光效（RGB 255,0,0，亮度0.7，闪烁频率1.2Hz）；镜头参数：长焦纪实分镜（焦距70mm，光圈f2.8），俯拍构图（视角45度向下）；细节参数：废墟质感（金属锈蚀程度60%，锈蚀纹理密度8根/cm²，布料破损面积20-30%，清晰可见）；环境参数：灰雾氛围（密度0.15，模糊半径1.8px），照度500lux，色温4000K；动态参数：人物动作（缓慢行走，步幅55cm，速度0.8m/s），废墟晃动（幅度0.25px，频率0.5Hz）；画面效果：充满末世苍凉与未来绝望感，阶级对比强烈，细节破败真实；必须保证4K超高清晰度"
        if subtype == "neon_asia":
            return "4K超高清东方霓虹赛博朋克（最高清晰度，霓虹与细节清晰锐利，16英寸屏幕绝对清晰），香港九龙寨城+东京涩谷融合，中式牌匾（字体大小3-5cm，字体风格宋体）+ 日文霓虹（字体大小2-4cm）；色彩参数：暖黄与冷紫撞色（RGB 255,200,100、RGB 128,0,255，饱和度85%，对比度180:1）；环境参数：潮湿巷弄（地面反射率85%，积水深度0.1-0.3cm，巷弄宽度2-3m）；细节参数：市井细节（商铺招牌密度15个/m，杂物堆积高度0.5-1.5m，线缆密度8根/m，清晰可见）；镜头参数：近景街头构图（焦距40mm，视角75度），镜头光晕（强度0.3，颜色RGB 255,200,100）；特效参数：霓虹光晕（半径2.0px，透明度70%），蒸汽特效（密度0.1，速度0.5m/s，温度色偏暖黄）；动态参数：行人动作（步幅60cm，速度1.0m/s），霓虹闪烁（频率2.2Hz），东方元素与未来科技完美融合；必须保证4K超高清晰度"
        return "16K UHD超高清赛博朋克2077夜之城风格（16英寸屏幕绝对清晰），UE5.3光线追踪渲染，最高清晰度"
    if style_type == "custom":
        v = image_style.get("value", "") or ""
        return f"自定义风格：{v}，4K超高清最高清晰度，16英寸屏幕绝对清晰" if v else "写实风格，8K超高清，最高清晰度，16英寸屏幕绝对清晰"
    return "写实风格，8K超高清，最高清晰度，细节丰富锐利，16英寸屏幕绝对清晰"
