...
def _generate_single_option(i: int, option: str, global_state: Dict) -> Dict:
    ...
    # 根据是否是第一次生成，调整场景描述要求
    if is_initial_scene:
        scene_requirement = """【场景】：开篇场景，第一印象入口。要求：开篇抓人、沉浸感强、留悬念，以主角第一视角叙事。

1. 开篇钩子：用「熟悉场景+异常细节+生存威胁」公式，开篇即抛出冲突或异常，直接进入危机感，3秒抓住注意力，有画面感。
2. 环境与感官：定基调、抓细节、融情绪；覆盖至少2–3种感官（视/听/嗅或触），动态细节优先，贴合悬疑或紧张基调。
3. 角色与内心：第一视角，写具体身体反应与心理挣扎，避免空洞的「感到害怕/紧张」。
4. 对话：2–3句对话，自然有张力，推动剧情、见人设；对话用引号，穿插动作与微表情。
5. 悬念：1个核心悬念+1个即时微悬念，具体有指向性，自然埋设，不刻意堆砌。
6. 世界观与主线：通过环境、对话、碎片记忆自然带出设定与主线暗示，不上帝视角讲解，不直说「主线是XX」。

全文流畅、有画面感，节奏「高压开场→细节铺陈→强钩子收尾」，让玩家看完想继续探索。"""
    else:
        scene_requirement = """【场景】：场景描述（必须是用户操作的直接结果，贴合难度和主角属性，要求：至少150字，包含环境描写、角色反应、对话等，对话必须使用引号）"""
    
    # 首段剧情：精简输入 token，缩短 LLM 响应时间（世界观/当前状态做字数裁剪）
    core_worldview_str = json.dumps(core_worldview, ensure_ascii=False)
    flow_worldline_str = json.dumps(flow_worldline, ensure_ascii=False)
    if is_initial_scene:
        core_worldview_str = _clip_text(core_worldview_str, 2400)
        flow_worldline_str = _clip_text(flow_worldline_str, 600)
    ...
    ## 【输入数据】：
    - 【核心世界观】：{core_worldview_str}
    - 【当前状态】：{flow_worldline_str}
